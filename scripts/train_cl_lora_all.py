#!/usr/bin/env python3
"""
Container-side training script adapted from old_fashion/notebooks/nuggets.ipynb.
It trains five independent LoRA adapters on the scored pythoncodes curriculum dataset.
Each adapter is trained as easy -> medium -> hard inside the same Docker container.
"""
...
class JsonlMetricCallback(TrainerCallback):
    ...
    def on_pre_optimizer_step(self, args, state, control, model=None, **kwargs):
        # Transformers often logs grad_norm itself, but this keeps the value available
        # even when the installed version does not include it in the log dict.
        if model is None:
            return
...
def env_int(name: str, default: int) -> int:
    """Read optional integer env var; empty/unset means default."""
...
def split_dataset(dataset: Dataset, cfg: dict[str, Any]) -> tuple[Dataset, Dataset]:
    ...
    val_size = int(ds_cfg.get("val_size", 1000))

    dataset = dataset.shuffle(seed=seed)
    val_size = min(val_size, max(1, len(dataset) // 10), len(dataset) - 1)
    val_dataset = dataset.select(range(val_size))
    train_dataset = dataset.select(range(val_size, len(dataset)))
    return train_dataset, val_dataset


def categories_for_stage(stages: list[str], stage: str, cumulative: bool) -> set[str]:
    if not cumulative:
        return {stage}
    pos = stages.index(stage)
    return set(stages[: pos + 1])



def prepare_prompt_dataset(dataset: Dataset, tokenizer, train: bool, seed: int) -> Dataset:
    """Convert scored pythoncodes rows to tokenized chat examples.

    The rows are still formatted as conversations. Labels are masked manually:
    system/user tokens are -100, assistant/output tokens are trainable.
    """
    mapped = dataset.map(
...
def filter_stage(dataset: Dataset, category_col: str, wanted: set[str]) -> Dataset:
    ...
    return dataset.filter(lambda row: row.get(category_col) in wanted)
...
def make_training_arguments(**kwargs) -> TrainingArguments:
    """Build TrainingArguments/SFTConfig across TRL and Transformers releases."""
    ...
    # TRL renamed the sequence-length field. Keep both forms in the call site and
    # retain whichever one the installed SFTConfig understands.
...
def make_sft_trainer(
    *,
    ...
    """Construct SFTTrainer across TRL/Unsloth API variants.

    The datasets are already tokenized and contain labels. Therefore we pass a
    normal data collator and disable TRL's own assistant_only_loss path to avoid
    double masking and the Unsloth raw-conversation formatting_func error.
    """
    ...
    for key, value in legacy_sft_kwargs.items():
        if key in params:
...
def train_one_run(
    ...
    if save_path.exists() and (save_path / "adapter_config.json").exists():
    ...
    with tee_to_file(LOG_ROOT / f"{run_name}.stdout.log"):
        logger.info("Starting run=%s category_col=%s", run_name, category_col)
        ...
        stages = list(ccfg.get("stages", ["easy", "medium", "hard"]))
        cumulative = bool(ccfg.get("cumulative", False))
        val_prompted = prepare_prompt_dataset(full_val_dataset, tokenizer=tokenizer, train=True, seed=seed)

        final_train_result = None
        if category_col is None:
            baseline_steps = int(run_cfg.get("max_steps", tcfg.get("max_steps", int(tcfg.get("stage_max_steps", 200)) * len(stages))))
            stage_plan = [(1, "all", {"all"}, full_train_dataset, baseline_steps)]
        else:
            stage_plan = []
            for stage_idx, stage in enumerate(stages, start=1):
                wanted = categories_for_stage(stages, stage, cumulative)
                stage_raw = filter_stage(full_train_dataset, category_col, wanted)
                stage_steps = int(run_cfg.get("stage_max_steps", tcfg.get("stage_max_steps", tcfg.get("max_steps", 200))))
                stage_plan.append((stage_idx, stage, wanted, stage_raw, stage_steps))

        for stage_idx, stage, wanted, stage_raw, max_steps_this_stage in stage_plan:
            if len(stage_raw) == 0:
                logger.warning("Stage %s skipped: empty dataset for %s in %s", stage, category_col, sorted(wanted))
                continue

            train_prompted = prepare_prompt_dataset(stage_raw, tokenizer=tokenizer, train=True, seed=seed + stage_idx)
            stage_name = f"{run_name}_stage_{stage_idx}_{stage}"
            stage_dir = run_out / stage_name
            stage_tb = tb_run / stage_name

            effective_max_steps = env_int("TRAIN_MAX_STEPS_OVERRIDE", max_steps_this_stage)
            stage_total = len(stage_plan)
            logger.info(
                "Stage %s/%s: %s | categories=%s | rows=%s | val_rows=%s",
                stage_idx,
                stage_total,
                stage,
                sorted(wanted),
            ...
            logger.info("Effective max_steps for this stage: %s", effective_max_steps)
            ...
            args = make_training_arguments(
                ...
                warmup_steps=int(tcfg.get("warmup_steps", 25)),
            ...
            trainer = make_sft_trainer(
                model=model,
                tokenizer=tokenizer,
                train_dataset=train_prompted,
                eval_dataset=val_prompted,
                args=args,
                tcfg=tcfg,
            ...
            logger.info("Stage %s final eval: %s", stage, eval_metrics)

            with (run_out / f"{stage_name}_final_eval.json").open("w", encoding="utf-8") as f:
                json.dump({k: _json_safe(v) for k, v in eval_metrics.items()}, f, indent=2, ensure_ascii=False)

            del trainer, train_prompted, stage_raw
            cleanup_cuda()
...
def main() -> int:
    ...
    parser.add_argument("--only", nargs="*", default=None, help="Optional run names, e.g. cl_length cl_semantic")
    ...
    for run_cfg in cfg["runs"]:
        if selected and run_cfg["name"] not in selected:
            continue
        cleanup_cuda()
        result = train_one_run(run_cfg, train_dataset, val_dataset, cfg, force=args.force)
