...
from src.data.curriculum.cluster.llm_judge_scoring import score_llm_judge_chunk
...
def main():
    ...
    if not bool(cfg.get("llm_judge", {}).get("enabled", True)):
        print("llm_judge.enabled=false, skip")
        return

    score_llm_judge_chunk(cfg, task_id=args.task_id)
    print("DONE")
