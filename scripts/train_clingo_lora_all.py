...
def prepare_tokenized_dataset(dataset: Dataset, tokenizer, train: bool, seed: int) -> Dataset:
    mapped = dataset.map(
        ...
        desc="Tokenizing clingo synthetic chats",
        keep_in_memory=True,
    )
    if train:
...
def normalize_report_to(value):
    ...
    if not lowered or any(x in {"", "none", "false", "0", "no", "off", "disabled"} for x in lowered):
        return []
...
def train_one_run(
    ...
    stage_adapter_root = run_out / "stage_adapters"
    ...
    stage_adapter_root.mkdir(parents=True, exist_ok=True)
    ...
    with tee_to_file(LOG_ROOT / f"{run_name}.stdout.log"):
        ...
        logger.info("Stage adapter root: %s", stage_adapter_root)
        ...
        for stage_idx, stage_spec in enumerate(stage_specs, start=1):
            ...
            try:
                ...
                stage_tb.mkdir(parents=True, exist_ok=True)
                report_to = normalize_report_to(tcfg.get("report_to", "none"))
                ...
                trainer = make_sft_trainer(
                    ...
                    args=args,
                    callbacks=[JsonlMetricCallback(metrics_jsonl, logger, tb_dir=(stage_tb if report_to else None))],
                )
...
def main() -> int:
    ...
    print(f"Loaded clingo dataset: train={len(train_dataset)}, val={len(val_dataset)}", flush=True)
